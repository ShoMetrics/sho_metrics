+++
title = "How do I make Stream Deck key colors look closer to my monitor?"
description = "Make ShoMetrics widget colors on physical Stream Deck keys look closer to your monitor."
weight = 20
aliases = ["/tutorials/color-compensation/"]
+++

You may notice that the colors you tuned on your monitor do not look the same on
the physical Stream Deck keys. As long as the difference is not extreme, that is
expected — the keys are small LCD displays with their own hardware limits, not a
sign that ShoMetrics got the color wrong.
