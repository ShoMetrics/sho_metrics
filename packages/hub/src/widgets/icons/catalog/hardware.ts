import { Battery, BatteryFull, BatteryLow, BatteryMedium, CircleQuestionMark, Cpu, Gpu, HardDrive, MemoryStick, Network } from "lucide";
...
export function getHardwareIconDefinition(kind: HardwareIconKind): SvgIconDefinition {
    switch (kind) {
        ...
        case "battery-empty":
            return createLucideIconDefinition({
                id: "hardware.battery.empty",
                node: BatteryLow,
                opticalScale: 1.08,
            });
        case "battery-full":
            return createLucideIconDefinition({
                id: "hardware.battery.full",
                node: BatteryFull,
                opticalScale: 1.08,
            });
        case "battery-medium":
            return createLucideIconDefinition({
                id: "hardware.battery.medium",
                node: BatteryMedium,
                opticalScale: 1.08,
            });
        case "unspecified":
