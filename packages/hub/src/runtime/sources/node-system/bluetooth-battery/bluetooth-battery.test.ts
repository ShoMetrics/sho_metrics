...
test("macOS Bluetooth descriptors use Stats-derived single battery readings", async () => {
    ...
    assert.equal(descriptors[0]?.displayName, "Demo Trackpad");
    ...
    assert.equal(descriptors[1]?.displayName, "Demo AirPods Pro Case");
    ...
    assert.equal(descriptors[2]?.displayName, "Demo AirPods Pro Left");
    assert.equal(descriptors[2]?.metricKey, airPodsLeftMetricKey);
    assert.equal(descriptors[3]?.displayName, "Demo AirPods Pro Right");
