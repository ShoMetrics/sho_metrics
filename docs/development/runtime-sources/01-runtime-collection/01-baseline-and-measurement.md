# Phases 1-4 Baseline And Measurement

This document records the runtime source performance work from May 2026 before
Phase 5 scheduler/runtime architecture work. It is focused on the Stream Deck
polling path, `node-system`, the Windows helper, and the measurement gate used
to decide whether an optimization is real.
...
- Refresh isolation gate: one slow collector must not block unrelated collectors
  or widgets. Pre-Phase 5 scheduler behavior did not satisfy this, because one
  slow source read could hold the shared interval group active and skip later
  ticks.
...
- Use ETW/WPR/PerfView when process attribution matters. PDH summaries are
...
## Current Situation Before Phase 5

The runtime still had a shared 1 Hz group stall problem after the Phase 1-4
cleanup. The scheduler coalesced same-interval metrics into one group. If one
collector path was slow, unrelated widgets in that group waited too.
...
- The refresh isolation gate is failing architecturally before Phase 5. The
  scheduler still grouped same-interval metrics, so a slow GPU/network/helper
  path could hold the whole 1 Hz group active. The latest run's `pollStartGap`
  max was 6060 ms, and the historical log already showed a 17890 ms group stall.
...
Release gate status for the latest Phase 4 run:
...
- Freshness target: FAIL. GPU usage sample age p95 was 3163 ms, above the 1500
  ms 1 Hz target.
- Refresh isolation gate: FAIL before Phase 5. Same-interval widgets could
  still wait behind one slow collector.
...
| Phase 3 | Added `RefreshableCache<T>` and `BackoffPolicy`. | Removed repeated ad-hoc async cache/backoff state machines from `node-system-source.ts`. |
| Phase 4 | Fixed CPU static info retry. | CPU usage stays independent of static CPU info failures; static info retries after transient failures. |

## Architectural Risks Retained From Phase 1-4
...
The network median improved, but GPU remains the strongest measured latency
suspect:
...
telemetry should avoid cold-spawning `nvidia-smi` per request. Candidate paths
are a long-lived `nvidia-smi --loop-ms=1000` reader or a direct API, but either
must be measured before replacing the current path.

The helper latency is not IPC latency. Local named pipe round trips should be
sub-millisecond. The 2-3s latency is consistent with synchronous
LibreHardwareMonitor traversal inside the request path. Caching helper pipe
responses would only hide the problem; caching the helper's latest hardware
snapshot would solve the hot response path.

## Why Phase 5 Exists

Phases 1-4 improved measurement, network median latency, stale-data correctness,
and duplicated cache code. They did not solve the bigger runtime shape:

```text
UI/Scheduler tick -> source I/O -> collector result -> render
```

Phase 5 exists because that chain lets slow collector I/O travel back into the
visible refresh path.
